...
void extractBones(const aiMesh* mesh, Skeleton& skeleton, std::vector<Vertex>& vertices)
{
...
void buildSkeletonNode(const aiNode* node, SkeletonNode& out)
{
...
std::shared_ptr<Texture> loadMaterialTexture(const aiMaterial* mat,
                                               aiTextureType type,
                                               ...
                                               bool          sRGB = false)
{
    ...
    return Texture::decodeFromFile(full.string(), engineType, true, sRGB);
}

std::shared_ptr<Material> processMaterial(const aiMaterial* mat, const fs::path& dir)
{
    ...
    if (auto t = loadMaterialTexture(mat, aiTextureType_DIFFUSE, TextureType::Albedo, dir, true))
        m->albedoMap = std::move(t);
    if (auto t = loadMaterialTexture(mat, aiTextureType_NORMALS, TextureType::Normal, dir))
        m->normalMap = std::move(t);
    if (auto t = loadMaterialTexture(mat, aiTextureType_METALNESS, TextureType::Metallic, dir))
        m->metallicMap = std::move(t);
    if (auto t = loadMaterialTexture(mat, aiTextureType_DIFFUSE_ROUGHNESS, TextureType::Roughness, dir))
        m->roughnessMap = std::move(t);
    if (auto t = loadMaterialTexture(mat, aiTextureType_AMBIENT_OCCLUSION, TextureType::AO, dir))
        m->aoMap = std::move(t);
    if (auto t = loadMaterialTexture(mat, aiTextureType_EMISSIVE, TextureType::Emissive, dir, true))
        m->emissiveMap = std::move(t);
...
Mesh processMesh(const aiMesh* mesh, const aiScene* /*scene*/, Model& model, const fs::path& /*dir*/)
{
...
void processNode(const aiNode* node, const aiScene* scene, Model& model, const fs::path& dir)
{
...
void extractAnimations(const aiScene* scene, Model& model)
{
    ...
    for (unsigned int a = 0; a < scene->mNumAnimations; ++a) {
        ...
        anim.ticksPerSecond = ai->mTicksPerSecond > 0.0 ? (float)ai->mTicksPerSecond : 25.0f;
...
std::shared_ptr<Model> ModelLoader::load(const std::string& path)
{
    ...
    constexpr unsigned int flags =
        ...
        | aiProcess_FlipUVs | aiProcess_JoinIdenticalVertices | aiProcess_LimitBoneWeights
    ...
    std::cout << "[ModelLoader] Loaded: " << path << "  meshes=" << model->meshes.size()
              << "  bones=" << model->skeleton->boneCount << "  anims=" << model->animations.size()
              << "\n";
